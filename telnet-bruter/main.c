...
void sockprintf(int fd, char *format, ...)
{
    ...
    ssize_t sent = send(fd, buffer, len, MSG_NOSIGNAL);
    if (sent == -1) {
        // Send failed, but we still need to free buffer
#ifdef DEBUG
        perror("send failed in sockprintf");
#endif
    }
    ...
    while(1)
    {
        ...
        for(int i = 0; i < nfds; i++)
        {
            ...
            else if(events[i].events & (EPOLLIN | EPOLLET))
            {
                ...
                if(brute->rdbuf_pos >= RDBUF_SIZE - 1)
                {
                    // Buffer is full, save last part for potential continuation
                    int keep_size = RDBUF_SIZE / 4;  // Keep last 25% of buffer
                    memmove(brute->rdbuf, brute->rdbuf + RDBUF_SIZE - keep_size, keep_size);
                    brute->rdbuf_pos = keep_size;
                    memset(brute->rdbuf + keep_size, 0, RDBUF_SIZE - keep_size);
#ifdef DEBUG
                    printf("Buffer overflow protection triggered for %s\n", brute->address);
#endif
                }
...
int main(int argc, char **argv)
{
    ...
    // Queue initialization is now handled in queue.c
    if (bruter_queue_init(999999) != 0) {
    ...
    while(1)
    {
        ...
        if(fgets(rdbuf, sizeof(rdbuf), stdin) == NULL)
            break;
